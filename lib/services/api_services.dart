...
class ApiService {
  ...
  static String get baseUrl {
    if (kIsWeb) {
      return "http://localhost/flutter_php"; // For Web (if using XAMPP)
    } else if (Platform.isAndroid) {
      return "http://10.0.2.2/flutter_php"; // For Android Emulator
    } else if (Platform.isIOS) {
      return "http://127.0.0.1/flutter_php"; // For iOS Simulator
    } else {
      return "http://192.168.138.12/flutter_php"; // For Physical Device (Use your local IP)
    }
  }
  ...
  static Future<bool> updateNote(
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/update_note.pxhp"),
